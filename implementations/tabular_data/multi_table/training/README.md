...
**CalavaDDPM uses a variation of TabDDPM for single-table modelling**: `ClavaDDPM` implements a faster version of `TabDDPM` where all the categorical values are Label-Encoded and treated like numerical values; therefore, it uses a single Gaussian diffusion backbone for both numerical and categorical data in a unified space. Another reason for this, as mentioned in the paper, is that the modeling of multinomial diffusion poses challenges to guided sampling in `ClavaDDPM`. This results in the eliminations of the multinomial diffusion loss in the loss term. This still work in practice because data distribution can be estimated and learned as a set of spikes at integers since a Gaussian diffusion model isn't restricted to smooth unimodal distributions and it can learn a distribution with mass at 0 and 5 and none in between.
...
$$
\mathbf{z}_i = \big[ \mathbf{x}^{\text{child}}*i \Vert \mathbf{x}^{\text{parent}}*{\pi(i)} \big]
$$
...
$$
\min_{y_i,\boldsymbol{\mu}*k}
\sum*{i=1}^{N}
\big\lVert \mathbf{z}*i - \boldsymbol{\mu}*{y_i} \big\rVert^2.
$$
...
### 3.1 TabDDPM variation as the backbone of single-table modelling

**IMOPRTANT NOTE**: `ClavaDDPM` implements a faster version of `TabDDPM` where all the categorical values are Label-Encoded and treated like numerical values; therefore, it uses a single Gaussian diffusion backbone for both numerical and categorical data in a unified space. Another reason for this, as mentioned in the paper, is that the modeling of multinomial diffusion poses challenges to guided sampling in `ClavaDDPM`. This results in the eliminations of the multinomial diffusion loss in the loss term. This still work in practice because data distribution can be estimated and learned as a set of spikes at integers since a Gaussian diffusion model isn't restricted to smooth unimodal distributions and it can learn a distribution with mass at 0 and 5 and none in between.

#### Regular TabDDPM loss
...
$$
\mathcal{L} = \mathcal{L}*{\text{multi}} + \mathcal{L}*{\text{gauss}}.
$$



#### TabDDPM variation loss used in ClavaDDPM
All categorical values are label-encoded and treated as numerical values.
...
$$
p_\phi(y \mid \mathbf{x}*t^{\text{num}}, t)
=
\mathrm{softmax}\big( f*\phi(\mathbf{x}_t^{\text{num}}, t) \big).
$$
...
At **synthesis**, this classifier is not used to pick a class after the fact. Its gradient $\nabla_{\mathbf{x}*t} \log p*\phi(y \mid \mathbf{x}_t, t)$ is added into the reverse diffusion step (scaled by `classifier_scale`) so child rows are steered toward the parent’s cluster. That is the same idea as classifier-guided image diffusion.
