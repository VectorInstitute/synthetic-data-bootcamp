...
def check_env_file(project_root: Path) -> CheckResult:
    ...
    if has_baseline:
        return CheckResult(
            "env file",
            "pass",
            "no local .env (using process env for API baseline)",
        )
    return CheckResult(
        "env file",
        "warn",
        f"no .env at {env_path}",
        f"optional: cp {example.name} .env for local API overrides / HF_TOKEN "
        "(Vector key is usually already in the workshop env)",
    )
...
def check_proxy_key() -> CheckResult:
    ...
    if not key:
        return CheckResult(
            ...
            "Workshop machines normally export these. For local override, set them in "
