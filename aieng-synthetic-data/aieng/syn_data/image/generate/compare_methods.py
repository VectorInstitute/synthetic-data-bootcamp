"""Side-by-side comparison of edge-case edit / generate methods (Notebook 0.5).

NB0.5 compares five edit paths; **production (NB1 / NB2) defaults to ``instruct``
(Klein)
...
5. ``vlm_generate_api`` — Nano Banana 2 / gemini-3.1-flash-image (opt-in;
   same Vector proxy + ``OPENAI_API_KEY`` as the API judge)
"""
...
from aieng.syn_data.image.config import merge_generation_anomaly, resolve_method_prompt
from aieng.syn_data.image.generate.conditioning import (
    DepthResult,
    SegmentationResult,
    build_anomaly_edit_mask,
    resolve_device,
)
from aieng.syn_data.image.generate.diffusers_klein import (
...
from aieng.syn_data.image.generate.variations import resolve_prompt_variation
from aieng.syn_data.image.generate.vlm_edit_local import (
    VlmLocalEditConfig,
    edit_with_qwen_local,
    unload_qwen_edit_pipeline,
)
from aieng.syn_data.image.generate.vlm_generate import (
    VlmGenerateConfig,
    generate_with_vlm,
...
# Large local + cloud instruction-class editors (NB0.5 only; not the NB1/NB2 default).
...
# Default NB0.5 grid — all five methods.
...
METHOD_SPECS: dict[str, MethodSpec] = {
    ...
    "vlm_generate_local": MethodSpec(
        ...
        summary=(
            ...
            "but heavier — NB0.5 comparison only. gpu_l4x2 recommended."
        ),
    ),
    "vlm_generate_api": MethodSpec(
        key="vlm_generate_api",
        title="Nano Banana 2 (Vector proxy image API)",
        uses_mask=False,
        uses_depth=False,
        uses_seg=False,
        summary=(
            "Cloud image edit via gemini-3.1-flash-image (aka Nano Banana 2) "
            "through the Vector OpenAI-compatible proxy (same OPENAI_API_KEY / "
            "proxy.vectorinstitute.ai as the judge). Opt in with INCLUDE_VLM_API=True."
        ),
    ),
    ...
    def compare_one(
        ...
        *,
        ...
        for method in methods:
            ...
            except Exception as exc:  # noqa: BLE001 — keep NB0.5 grid alive on OOM / disk
